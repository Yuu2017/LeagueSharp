...
namespace HikiCarry.Core.Utilities
{
    class KalistaLogics
    {
        ...
        {
            foreach (var support in ObjectManager.Get<Obj_AI_Hero>().Where(x=> x.IsAlly && !x.IsMe && x.Distance(ObjectManager.Player.Position) < Kalista.R.Range &&
        ...
        {
            foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsAlly && !x.IsMe && x.HasBuff("kalistacoopstrikeally") && x.Distance(ObjectManager.Player.Position) < Kalista.R.Range &&
