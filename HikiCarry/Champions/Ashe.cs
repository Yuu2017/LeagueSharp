...
namespace HikiCarry.Champions
{
    internal class Ashe
    {
        ...
        public Ashe()
        {
            ...
            Obj_AI_Base.OnDoCast += AsheOnDoCast;
        ...
        {
            if (sender.IsEnemy && sender is Obj_AI_Hero && args.IsDash && Utilities.Enabled("dash.block")
        ...
        private void AsheOnDoCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe && args.Target is Obj_AI_Hero && args.SData.IsAutoAttack())
            {
                if (Q.IsReady() && Utilities.Enabled("q.combo") && ((Obj_AI_Hero)args.Target).IsValidTarget(ObjectManager.Player.AttackRange) &&
                    sender.HasBuff("asheqcastready") && Initializer.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
                {
                    Q.Cast();
                }

                if (Q.IsReady() && Utilities.Enabled("q.harass") && ((Obj_AI_Hero)args.Target).IsValidTarget(ObjectManager.Player.AttackRange) &&
