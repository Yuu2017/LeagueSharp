...
namespace HikiCarry.Champions
{
    class Lucian
    {
        ...
        private float TotalDamage(Obj_AI_Hero hero)
        {
            var damage = 0d;
            if (Q.IsReady())
            {
        ...
        private static void HarassQCast()
        {
            switch (Initializer.Config.Item("lucian.q.type",true).GetValue<StringList>().SelectedIndex)
            {
                case 0:
                    ...
                    var target = ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsValidTarget(Q2.Range)).FirstOrDefault(x => Initializer.Config.Item("lucian.white" + x.ChampionName,true).GetValue<bool>());
        ...
        private void LucianOnDoCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe && Orbwalking.IsAutoAttack(args.SData.Name) && args.Target is Obj_AI_Hero && args.Target.IsValid)
            {
                if (Utilities.Enabled("lucian.combo.start.e"))
                {
                    ...
                    {
                        Q.CastOnUnit(((Obj_AI_Hero)args.Target));
                    }
                        ...
                        Initializer.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo && ObjectManager.Player.Buffs.Any(buff => buff.Name != "lucianpassivebuff"))
                    {
                        ...
                        {
                            W.Cast(((Obj_AI_Hero)args.Target).Position);
                        }
                        else
                        {
                            if (W.GetPrediction(((Obj_AI_Hero)args.Target)).Hitchance >= HitChance.Medium)
                            {
                                W.Cast(((Obj_AI_Hero)args.Target).Position);
                            }
                        }
                        ...
                        Initializer.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo && ObjectManager.Player.Buffs.Any(buff => buff.Name != "lucianpassivebuff"))
                    {
                        ...
                        {
                            case 0:
                                Utilities.ECast(((Obj_AI_Hero)args.Target), E);
                                break;
                            case 1:
                ...
                else
                {
                    ...
                    if (W.IsReady() && Utilities.Enabled("lucian.w.combo") &&
                        ObjectManager.Player.Distance(args.Target.Position) < W.Range &&
                        Initializer.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo && ObjectManager.Player.Buffs.Any(buff => buff.Name != "lucianpassivebuff")
                        && W.GetPrediction(((Obj_AI_Hero)args.Target)).Hitchance >= HitChance.Medium)
                    {
                        W.Cast(((Obj_AI_Hero)args.Target).Position);
                    }
                    if (E.IsReady() && Utilities.Enabled("lucian.e.combo") &&
